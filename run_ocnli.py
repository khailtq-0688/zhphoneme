    ...
    with open(data_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            try:
                ...
                # TỐI ƯU 1: Lọc độ dài câu tiền đề chuẩn theo bài báo (8 đến 50 ký tự)
                if is_training and (len(s1) < 8 or len(s1) > 50):
                    continue

                if is_training:
                    label_str = data.get("label", "-")
                    # TỐI ƯU 2: Loại bỏ triệt để các nhãn "-" (không đồng thuận)
...
def convert_examples_to_features(examples, tokenizer, max_seq_length, is_training=False):
    ...
    for example in tqdm(examples, desc="Converting features"):
        ...
        # # TỐI ƯU BỎ NHIỄU: Nếu là tập Train mà câu dài quá, vứt luôn mẫu này
        # if is_training and (len(tokens_s1) + len(tokens_s2) > max_seq_length - 3):
        #     continue

        # TỐI ƯU 3: Chỉ cắt phần đuôi của câu tiền đề (tokens_s1), bảo toàn tokens_s2
        while len(tokens_s1) + len(tokens_s2) > max_seq_length - 3:
            if len(tokens_s1) > 1: # Đảm bảo không cắt hết tokens_s1
                tokens_s1.pop()
            else:
                tokens_s2.pop() # Trường hợp cực kỳ bất đắc dĩ
