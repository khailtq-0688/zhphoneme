...
class PinyinBertConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        pad_token_id=0,
        is_decoder=False,           # Thêm giá trị mặc định
        add_cross_attention=False,  # Thêm giá trị mặc định
        **kwargs,
    ):
        pad_token_id = kwargs.pop("pad_token_id", pad_token_id)
        is_decoder = kwargs.pop("is_decoder", is_decoder)
        super().__init__(pad_token_id=0, is_decoder=is_decoder, **kwargs)
