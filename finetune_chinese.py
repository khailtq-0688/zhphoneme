#!/usr/bin/env python3
"""
Chinese Language-Specific Fine-tuning Script
Loads pretrained multilingual model and fine-tunes on Chinese dataset
"""
...
from data_processing.data_processor import ChineseProcessor, DataMerger
...
def process_chinese_dataset(config):
    """Process Chinese dataset only (no merging)"""
    data_config = config.get('data_paths', {})
    chinese_config = data_config.get('chinese', {})
    preprocessing_config = config.get('preprocessing', {})
    
    output_dir = preprocessing_config.get('output_dir', './processed_data/chinese_finetune')
    ...
    logger.info("="*60)
    logger.info("Processing Chinese Dataset for Fine-tuning")
    logger.info("="*60)
...
def main():
    parser = ArgumentParser(description='Chinese Fine-tuning for ViWordFormer')
    parser.add_argument(
        '--config',
        type=str,
    ...
    if not args.no_process:
        ...
        processed_dir = config.get('preprocessing', {}).get('output_dir', './processed_data/chinese_finetune')
    ...
    logger.info("="*60)
    logger.info("Starting Chinese Fine-tuning")
    logger.info("="*60)
    ...
    finetune_config = dict_to_dotdict({
        ...
        'checkpoint_dir': Path('./checkpoints/chinese_finetune'),
    ...
    logger.info("="*60)
    logger.info("Chinese Fine-tuning Complete!")
