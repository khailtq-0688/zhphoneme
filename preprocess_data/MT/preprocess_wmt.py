...
SYMBOL_MAP = {
    ...
    '√': '根号',
    '㎡': '平方米',
    '/': '每', # Dịch là "mỗi" (per) cho ngữ cảnh đơn vị đo lường
}
...
def process_line(item):
    ...
    if not re.search(f'[{HANZI_RANGE}a-zA-Z]', text):
        return None, "NO_CONTENT_FOUND", removed_chars_in_this_line
...
def preprocess_wmt_dataset(input_path, output_dir):
    ...
    with open(input_path, 'r', encoding='utf-8') as fin, \
        ...
        for line in tqdm(fin, desc="Processing"):
            ...
            if status == "OK":
                stats["kept"] += 1
                item[TARGET_FIELD] = processed_text
                if "is_bad_source" in item: del item["is_bad_source"]
                f_pure.write(json.dumps(item, ensure_ascii=False) + '\n')
            else:
                stats["rejected"] += 1
    ...
    report = {
        ...
        "stats": {
            ...
            "kept_ratio": f"{(stats['kept']/stats['total']*100):.2f}%"
        },
        "reasons": stats["reasons"],
        "analysis": { "symbol_removed": dict(counter_removed_garbage.most_common()) }
    }
    
    with open(out_report, 'w', encoding='utf-8') as f_rep:
        json.dump(report, f_rep, indent=4, ensure_ascii=False)

    print(f"\n✅ HOÀN TẤT! File sạch: {out_pure}")
