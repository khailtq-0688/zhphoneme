...
def process_ocnli_file(input_path, output_dir):
...
if __name__ == "__main__":
    INPUT_FILES = ["train.50k.json", "dev.json", "test.json"]
    OUTPUT_DIR = "ocnli_output"
    if not os.path.exists(OUTPUT_DIR): os.makedirs(OUTPUT_DIR)
    for file_name in INPUT_FILES:
        if os.path.exists(file_name): process_ocnli_file(file_name, OUTPUT_DIR)
    print(f"\n✅ HOÀN TẤT: {OUTPUT_DIR}")