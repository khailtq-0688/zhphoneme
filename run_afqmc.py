...
def convert_examples_to_features(examples, tokenizer, max_seq_length):
    ...
    for example in tqdm(examples, desc="Converting features"):
        tokens_a = tokenizer.encode(example["sentence1"]).tolist()[1:]
        tokens_b = tokenizer.encode(example["sentence2"]).tolist()[1:]
...
def train(args):
    ...
    logger.info("Đang đọc dữ liệu AFQMC...")
    train_examples = load_afqmc_data(os.path.join(args.data_dir, "train.json"))
    dev_examples = load_afqmc_data(os.path.join(args.data_dir, "dev.json"))

    logger.info(f"Train: {len(train_examples)} | Dev: {len(dev_examples)}")
    ...
    train_dataloader = create_dataloader(train_features, args.train_batch_size, True)
    dev_dataloader = create_dataloader(dev_features, args.eval_batch_size, False)
    ...
    logger.info("***** Bắt đầu tiến trình Huấn luyện AFQMC *****")
    ...
    for epoch in range(args.epochs):
        ...
        if eval_acc > best_acc:
            best_acc = eval_acc
            save_path = os.path.join(args.output_dir, "best_model_afqmc.pt")

            torch.save(model.state_dict(), save_path)

            logger.info(
                f"✨ Saved best model (Acc: {best_acc*100:.2f}%) -> {save_path}"
            )

    logger.info(f"🏁 Best Eval Accuracy: {best_acc*100:.2f}%")

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", type=str, required=True, help="Thư mục chứa dữ liệu AFQMC (train.json, dev.json)")
    ...
    parser.add_argument("--output_dir", type=str, default="./afqmc_outputs", help="Nơi lưu checkpoint mô hình")
