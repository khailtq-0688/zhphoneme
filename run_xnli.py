...
# xnli có 3 nhãn tiêu chuẩn: entailment, neutral, contradiction
...
def load_xnli_data(data_path, is_training=True):
    examples = []
...
def convert_examples_to_features(examples, tokenizer, max_seq_length, is_training=False):
    ...
    for example in tqdm(examples, desc="Converting features"):
        ...
        if len(tokens_s1) > (max_seq_length//2 - 1):
            tokens_s1 = tokens_s1[:(max_seq_length//2 - 1)]
        if len(tokens_s2) > (max_seq_length//2 - 1):
            tokens_s2 = tokens_s2[:(max_seq_length//2 - 1)]

        input_ids = [cls_token] + tokens_s1 + [pad_token] + tokens_s2
        
        part1_len = len(tokens_s1) + 1
...
def train(args):
    ...
    cache_train_path = os.path.join(args.data_dir, f"cached_xnli_train_{args.max_seq_length}.pt")
    cache_dev_path = os.path.join(args.data_dir, f"cached_xnli_dev_{args.max_seq_length}.pt")
    cache_test_path = os.path.join(args.data_dir, f"cached_xnli_test_{args.max_seq_length}.pt")

    # --- XỬ LÝ CACHE CHO TẬP TRAIN ---
    if os.path.exists(cache_train_path):
        logger.info(f"👉 Tìm thấy cache dữ liệu xnli Train, đang tải từ: {cache_train_path}")
        train_features = torch.load(cache_train_path)
    else:
        logger.info("❌ Không tìm thấy cache Train. Bắt đầu convert từ file JSON gốc...")
        train_examples = load_xnli_data(os.path.join(args.data_dir, "train.json"), is_training=True)
    ...
    dev_examples = load_xnli_data(dev_file_path, is_training=True)

    if os.path.exists(cache_dev_path):
        logger.info(f"👉 Tìm thấy cache dữ liệu xnli Dev, đang tải từ: {cache_dev_path}")
    ...
    # --- XỬ LÝ CACHE CHO TẬP TEST (ĐÃ ĐƯỢC CHỈNH SỬA TỐI ƯU) ---
    test_file_path = os.path.join(args.data_dir, "test.json")
    
    # GIẢI PHÁP LUÔN TẢI DỮ LIỆU THÔ: 
    # Việc đọc file JSON này rất nhanh, đảm bảo luôn có `test_examples` cho quá trình xuất Bad Cases
    logger.info(f"📋 Đang nạp dữ liệu Test: {test_file_path}")
    test_examples = load_xnli_data(test_file_path, is_training=True)

    if os.path.exists(cache_test_path):
        logger.info(f"👉 Tìm thấy cache dữ liệu xnli Dev, đang tải từ: {cache_test_path}")
        test_features = torch.load(cache_test_path)
    else:
        logger.info("❌ Không tìm thấy cache Test. Bắt đầu mã hóa từ dữ liệu thô...")
        logger.info(f"Số lượng mẫu kiểm thử thô hợp lệ: {len(test_examples)}")
        test_features = convert_examples_to_features(test_examples, tokenizer, args.max_seq_length, is_training=False)
        logger.info(f"💾 Đang lưu dữ liệu Test đã convert vào cache: {cache_test_path}")
    ...
    logger.info("***** Bắt đầu tiến trình Huấn luyện xnli *****")
    best_acc = 0.0
    ...
    for epoch in range(args.epochs):
        ...
        if eval_acc > best_acc:
            best_acc = eval_acc
            save_path = os.path.join(args.output_dir, "best_model_xnli.pt")
...
def main():
    ...
    parser.add_argument("--data_dir", type=str, required=True, help="Thư mục chứa dữ liệu xnli (train.json, dev.json)")
    parser.add_argument("--init_checkpoint", type=str, required=True, help="Thư mục weights gốc pre-train (chứa config.json)")
    parser.add_argument("--output_dir", type=str, default="./xnli_outputs", help="Thư mục lưu mô hình sau huấn luyện")
