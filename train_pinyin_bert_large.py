...
if RESUME_FROM_CHECKPOINT and os.path.isfile(checkpoint_path):
    print(f"=> Tìm thấy checkpoint Large. Đang tiến hành khôi phục từ: {checkpoint_path}")
    ...
    print(f"=> Không kích hoạt resume hoặc không tìm thấy file checkpoint. Khởi chạy tiền huấn luyện Large mới từ đầu.")
...
# Vòng lặp Step-based vô hạn sử dụng while True đồng bộ theo cấu trúc mô hình lớn
while True:
    total_loss = 0
    ...
    for batch in progress_bar:
        ...
        # Huấn luyện độ chính xác hỗn hợp AMP (Rất quan trọng để tránh OOM bộ nhớ đối với bản Large)
        ...
        # Cơ chế lưu trạng thái huấn luyện chi tiết sau mỗi 1000 steps
