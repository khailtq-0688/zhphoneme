...
def load_thucnews_data(data_path, is_training=True):
    examples = []
...
def create_dataloader(features, batch_size, is_training=True):
    ...
    all_labels = torch.tensor([LABEL_MAP[f["label"]] for f in features], dtype=torch.long)
...
def train(args):
    ...
    if os.path.exists(cached_train_file):
        ...
        train_examples = load_thucnews_data(os.path.join(args.data_dir, "cnews.train.txt"))
    ...
    if os.path.exists(cached_dev_file):
        ...
        dev_examples = load_thucnews_data(os.path.join(args.data_dir, "cnews.val.txt"))
    ...
    if os.path.exists(cached_test_file):
        ...
        test_examples = load_thucnews_data(os.path.join(args.data_dir, "cnews.test.txt"))
    ...
    logger.info("***** Bắt đầu tiến trình Huấn luyện THUCNews *****")
    best_acc = 0.0
    ...
    for epoch in range(args.epochs):
        ...
        if eval_acc > best_acc:
            best_acc = eval_acc
            save_path = os.path.join(args.output_dir, "best_model_thucnews.pt")
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="checkpoints/THUCNews", help="Thư mục lưu mô hình sau huấn luyện")
