...
def load_tnews_data(data_path, label2id):
    examples = []
...
def convert_examples_to_features(examples, tokenizer, max_seq_length, label2id):
...
def train(args):
    ...
    train_examples = load_tnews_data(os.path.join(args.data_dir, "train_filtered.json"), label2id)
    dev_examples = load_tnews_data(os.path.join(args.data_dir, "dev.json"), label2id)

    logger.info(f"Số lượng train: {len(train_examples)} | dev: {len(dev_examples)}")

    train_features = convert_examples_to_features(
        train_examples, tokenizer, args.max_seq_length, label2id
    )
    dev_features = convert_examples_to_features(
        dev_examples, tokenizer, args.max_seq_length, label2id
    )
    ...
    logger.info("***** Bắt đầu tiến trình Huấn luyện TNEWS *****")
    ...
    for epoch in range(args.epochs):
        ...
        if eval_acc > best_acc:
            best_acc = eval_acc
            save_path = os.path.join(args.output_dir, "best_model_tnews.pt")
