...
# ==========================================
# KIẾN TRÚC V3: KẾT HỢP [CLS] + MEAN POOLING
# ==========================================
class ViPhonBertForSequenceClassification(nn.Module):
    def __init__(self, config, num_labels, class_weights=None):
        ...
        self.dropout = nn.Dropout(0.15) # Tăng dropout nhẹ để chống overfit
        
        # Vì ta sẽ nối (cat) CLS và Mean Pooling lại nên input sẽ là hidden_size * 2
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        ...
        # 1. Trích xuất [CLS] token (Tốt cho suy luận NLI)
        cls_output = sequence_output[:, 0, :]
        
        # 2. Trích xuất Mean Pooling (Tốt cho bao quát ngữ cảnh)
        ...
        # 3. Kết hợp 2 đặc trưng (Concatenation)
        ...
        if labels is not None:
            # Hạ Label Smoothing xuống 0.01 để model tự tin hơn khi phán đoán
            loss_fct = nn.CrossEntropyLoss(weight=self.class_weights, label_smoothing=0.01)
...
def train(args):
    ...
    if os.path.exists(cache_train_path): train_features = torch.load(cache_train_path)
    else:
    ...
    if os.path.exists(cache_dev_path): dev_features = torch.load(cache_dev_path)
    else:
    ...
    if os.path.exists(cache_test_path): test_features = torch.load(cache_test_path)
    else:
    ...
    logger.info("***** Bắt đầu tiến trình Huấn luyện VNLI V3 *****")
    best_f1 = 0.0
    ...
    if test_dataloader is not None and os.path.exists(best_model_path):
        ...
        logger.info(f"🎯 KẾT QUẢ TEST CUỐI CÙNG VNLI - Loss: {test_loss:.4f}")
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./vnli_outputs")
