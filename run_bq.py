...
INV_LABEL_MAP = {id: label for label, id in LABEL_MAP.items()}
NUM_LABELS = 2
...
def load_bq_data(data_path, is_training=True):
    examples = []
...
def convert_examples_to_features(examples, tokenizer, max_seq_length):
    ...
    for example in tqdm(examples, desc="Converting features"):
        tokens1 = tokenizer.encode(example["sentence1"]).tolist()[1:]
        tokens2 = tokenizer.encode(example["sentence2"]).tolist()[1:]

        input_ids1 = [cls_token] + tokens1
        input_ids2 = [pad_token] + [pad_token] + tokens2
...
def train(args):
    ...
    cached_test_file = os.path.join(args.data_dir, f"cached_test_features_{args.max_seq_length}.pt")
    ...
    if os.path.exists(cached_train_file):
        ...
        train_examples = load_bq_data(os.path.join(args.data_dir, "train.json"))
    ...
    if os.path.exists(cached_dev_file):
        ...
        dev_examples = load_bq_data(os.path.join(args.data_dir, "dev.json"))
        dev_features = convert_examples_to_features(dev_examples, tokenizer, args.max_seq_length)
        logger.info(f"💾 Đang lưu Dev features vào cache để dùng cho lần sau: {cached_dev_file}")
        torch.save(dev_features, cached_dev_file)
    
    if os.path.exists(cached_test_file):
        logger.info(f"⚡ Đã tìm thấy file cache! Đang tải nhanh Test features từ: {cached_test_file}")
        test_features = torch.load(cached_test_file)
    else:
        test_examples = load_bq_data(os.path.join(args.data_dir, "test.json"))
        test_features = convert_examples_to_features(test_examples, tokenizer, args.max_seq_length)
        logger.info(f"💾 Đang lưu Test features vào cache để dùng cho lần sau: {cached_test_file}")
        torch.save(test_features, cached_test_file)
    ...
    logger.info("***** Bắt đầu tiến trình Huấn luyện BQ *****")
    best_acc = 0.0
    ...
    for epoch in range(args.epochs):
        ...
        if eval_acc > best_acc:
            best_acc = eval_acc
            save_path = os.path.join(args.output_dir, "best_model_bq.pt")
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="checkpoints/BQ", help="Thư mục lưu mô hình sau huấn luyện")
    
    parser.add_argument("--max_seq_length", type=int, default=256, help="Độ dài chuỗi tối đa tích hợp cặp câu")
    parser.add_argument("--train_batch_size", type=int, default=64, help="Kích thước batch size huấn luyện")
    parser.add_argument("--eval_batch_size", type=int, default=32, help="Kích thước batch size đánh giá")
    parser.add_argument("--learning_rate", type=float, default=5e-5, help="Tốc độ học tối ưu ban đầu")
